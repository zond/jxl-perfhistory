...
fn print_results_single_markdown(results: &[Revision], args: &Args, noise_metrics: &NoiseMetrics) {
    ...
    // Format commit URLs
    let head_url = if let Some(ref url) = github_url {
        format!("{}/commit/{}", url, head_rev.oid)
    } else {
        format!("{:.8}", head_rev.oid)
    };
    let main_url = if let Some(ref url) = github_url {
        format!("{}/commit/{}", url, main_rev.oid)
    } else {
    ...
    // Results table with commit URLs in header
    println!("| {} vs {} | Main | PR | % Diff |", head_url, main_url);
    println!("|---|---|---|---|");
    println!("| {} | {:.3} | {:.3} | {} |", file_name, main_mps, head_mps, format_diff(pct_diff));
}
...
fn print_results_multifile_markdown(
    ...
    let num_files = head_rev.file_results.len();
    ...
    for (file_idx, head_fr) in head_rev.file_results.iter().enumerate() {
        ...
        if let (Some(main_median), Some(head_median)) = (main_fr.median, head_fr.median) {
            ...
            println!("| {} | {:.3} | {:.3} | {} |",
                     file_name, main_mps, head_mps, format_diff(pct_diff));
        }
    }
}
